...
python:
  executable: "C:/Users/gstarrs/Projects/IGIS/Quarto_Python/.conda/python.exe"
...
# Folder with images
img_folder = r"image_assets\staff_circles"
img_files = [os.path.join(img_folder, f) for f in sorted(os.listdir(img_folder)) if f.endswith(".png")]
...
# Generate HTML for a staff row with larger images
html = '<div style="display: flex; justify-content: center; align-items: flex-start; gap: 15px;">'  # more gap for bigger images
for img, label in zip(img_files, labels):
    # Split first and last names to force line break
    label_html = '<br>'.join(label.split())
    html += f'''
    <div style="
        text-align: center; 
        font-size: 24px;       /* slightly larger font */
        font-weight: 500;      /* moderately bold */
        line-height: 1.2em;    /* spacing between lines */
        color: #005fae;
    ">
        <img src="{img}" width="130px" style="border-radius: 50%;"><br>  <!-- larger image -->
        {label_html}
    </div>
    '''
html += '</div>'
